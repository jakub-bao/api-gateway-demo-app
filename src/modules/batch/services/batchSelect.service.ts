...
function selectPageOnly(dedupes:DedupeModel[]){
    let ids:number[] = [...document.querySelectorAll('[data-type="batch_checkbox"]')].map(c=>parseInt(c.getAttribute('data-id')));
    dedupes.filter(d=>d.status!==InternalStatus.unresolvableCrosswalk).filter(d=>ids.includes(d.meta.internalId)).forEach((d:DedupeModel)=>{
...
export function batchSelectDedupes(dedupes:DedupeModel[], selectionType:SelectionType):DedupeModel[]{
    switch(selectionType){
        case SelectionType.allMatching:
            dedupes.filter(d=>d.status!==InternalStatus.unresolvableCrosswalk).forEach(d=>d.tableData.checked=true);
            break;

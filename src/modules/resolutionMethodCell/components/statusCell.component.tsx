...
function getStatusStyle(status:InternalStatus):object{
    let color;
    if (![InternalStatus.unresolved, InternalStatus.unresolvableCrosswalk].includes(status)) color = 'white';
    return Object.assign({},styles.status,{color:color});
}

function StatusText({status}:{status:InternalStatus}){
    let props:{title:string, open?:boolean, children:any} = {title: 'Please resolve pure dedupe first', children:null};
    if (status!==InternalStatus.unresolvableCrosswalk) props.open = false;
    return React.createElement(Tooltip,props,<Typography style={getStatusStyle(status)}>{statusToText(status)}</Typography>);
}
...
export default function StatusCell({dedupe, resolveDedupe, unresolveDedupe}:{dedupe:DedupeModel, resolveDedupe:ResolveDedupe, unresolveDedupe:UnresolveDedupe}) {
    ...
    return <div style={styles.root} data-testid={`status_${dedupe.meta.internalId}`}>
        {![InternalStatus.unresolved,InternalStatus.unresolvableCrosswalk].includes(dedupe.status) && <div style={styles.spacer}/>}
        <StatusText status={dedupe.status}/>

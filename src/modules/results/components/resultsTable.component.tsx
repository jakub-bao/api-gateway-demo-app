...
import {isTestEnv} from "@dhis2-app/api";
...
const tableOptions:Options<DedupeModel> = {
    ...
    draggable: false,
    selectionProps: (data:DedupeModel)=>({'data-testid':`batch_checkbox_${data.meta.internalId}`,'data-id':data.meta.internalId,'data-type':'batch_checkbox', 'data-style':'batch_checkbox', disabled: data.status===InternalStatus.unresolvableCrosswalk}),
...
const ResolutionHeader = ({dedupeType}:{dedupeType:DedupeType})=><DedupeTooltip title={tooltips[dedupeType.toLocaleLowerCase()].resolution}><span>Resolution</span></DedupeTooltip>

const getColumnSettings = (setResolutionValue:SetResolutionValue, changeResolutionMethod:ChangeResolutionMethod, resolveDedupe: ResolveDedupe, unresolveDedupe: UnresolveDedupe, dedupeType: DedupeType)=> [
    {
        ...
        title: <RespText long='Disaggregation' short='Disagg' cutoff={1200}/>,
        ...
        title: <ResolutionHeader dedupeType={dedupeType}/>,
...
export default function ResultsTable({filteredDedupes, setResolutionValue, changeResolutionMethod, resolveDedupe, unresolveDedupe,onSelectChange,dedupeType}:{
    ...
    return <MaterialTable
        ...
        columns={getColumnSettings(setResolutionValue, changeResolutionMethod, resolveDedupe, unresolveDedupe, dedupeType)}

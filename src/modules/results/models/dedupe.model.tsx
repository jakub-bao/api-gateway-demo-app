...
export enum InternalStatus{
    ...
    unresolvableCrosswalk='unresolvableCrosswalk'
};
...
const isResolved = (dedupe:DedupeModel)=>dedupe.resolution&&dedupe.resolution.resolutionMethodValue;
const checkUnresolvableCrosswalk = (dedupe:DedupeModel)=>{
    try {
        return dedupe.duplicates.filter(d=>['DSD Value','TA Value'].includes(d.agencyName)&&d.value===null).length>0;
    } catch(e){
        return false;
    }
};
...
function getDedupeStatus(dedupe:DedupeModel):InternalStatus{
    if (checkUnresolvableCrosswalk(dedupe)) return InternalStatus.unresolvableCrosswalk;
    if (!isResolved(dedupe)) return InternalStatus.unresolved;

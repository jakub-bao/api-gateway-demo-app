...
import {getResolutionValue} from "../../resolutionMethodCell/services/getResolutionValue.service";
import {getAdjustmentValueByMethod} from "../../resolutionMethodCell/services/getAdjustmentValue.service";
...
function extractDuplicates(rows:namedRow[]):DuplicateModel[]{
    ...
    }).sort((a,b)=>a.mechanismNumber>b.mechanismNumber?1:-1)
}
...
function generateDedupe(selectedRows: namedRow[], groupNumber:number, filters:FiltersModel):DedupeModel{
    ...
    updateStatus(dedupe);
    preselectCrosswalks(dedupe);
    if (dedupe.duplicates.map(d=>d.value).includes(494)) {
        console.log(dedupe);
...
function preselectCrosswalks(dedupe:DedupeModel){
    if (dedupe.meta.dedupeType===DedupeType.pure) return;
    if (dedupe.status!==InternalStatus.unresolved) return;
    dedupe.resolution.resolutionMethodValue = {
        resolutionValue: getResolutionValue(dedupe,ResolutionMethodType.maximum),
        resolutionMethod: ResolutionMethodType.maximum,
        deduplicationAdjustmentValue: getAdjustmentValueByMethod(dedupe, ResolutionMethodType.maximum)
    }
...
const getValue = (orig:string)=>{
    if (orig==='unresolved') return null;
    else return parseInt(orig);
};
